defmodule SREChatWeb.WSHandler do
  @moduledoc false
  @behaviour :cowboy_websocket

  require Logger

  alias SREChat.{Config, Observability, Store}
  ...
  @impl true
  def websocket_info({:sre_chat_system_event, %{"type" => "membership_changed"}}, state),
  ...
  defp replace_user_subscription(state, uid) do
    case Map.get(state, :uid) do
      previous_uid when is_binary(previous_uid) and previous_uid != "" and previous_uid != uid ->
        SREChat.PubSub.unsubscribe({:user, previous_uid})

      _other ->
        :ok
    end

    SREChat.PubSub.unsubscribe({:user, uid})
    subscribe({:user, uid})
    state
  end
  ...
  defp sync_group_subscriptions(%{uid: uid} = state) when is_binary(uid) and uid != "" do
    ...
    case Store.groups_for_user(uid) do
      {:ok, groups} ->
        ...
        |> Enum.each(&SREChat.PubSub.unsubscribe({:group, &1}))
  ...
  defp subscribe(key) do
    case SREChat.PubSub.subscribe(key) do
      {:ok, _pid} -> :ok

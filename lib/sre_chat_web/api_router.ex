defmodule SREChatWeb.ApiRouter do
  @moduledoc false
  use Plug.Router
  import Plug.Conn
  alias SREChat.{Config, Errors, Media, Store, Time}
  alias SREChat.Store.AuthTokens
  alias SREChatWeb.{ApiResponse, Auth, JSON}
  ...
  match "/*path" do
    if extension_host?(conn) do
      ...
      JSON.error(
        ...
        Errors.error(
          "ERR_ROUTE_NOT_FOUND",
          "No SREChat route matched #{conn.method} #{conn.request_path}"
        ),
        404
      )
    end
  ...
  defp wait_for_dm_history_connect_grace do
    case Config.dm_history_connect_grace_ms() do
      ms when is_integer(ms) and ms > 0 ->
        ...
        # fetchPrevious(); this keeps SREChat from outrunning the SDK's

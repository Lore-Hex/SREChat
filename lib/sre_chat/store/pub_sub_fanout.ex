defmodule SREChat.Store.PubSubFanout do
  @moduledoc false

  alias SREChat.{Config, Media, Time}
  alias SREChat.Store.{MessageData, MessageState}
  ...
  def message(state, message, opts \\ []) do
    ...
    SREChat.PubSub.broadcast(recipient_keys(state, message), event)
  end
  ...
  def message_update(state, message, actor_uid \\ nil, action_id \\ nil, opts \\ []) do
    ...
    SREChat.PubSub.broadcast(update_recipient_keys(state, message), event)
  end
  ...
  def receipt(payload, uid, receiver_type, receiver_id, action, opts \\ []) do
    ...
    SREChat.PubSub.broadcast(targets, event)
  end
  ...
  def group_action(state, guid, action, opts) do
    ...
    SREChat.PubSub.broadcast(keys, event)
  end
  ...
  def membership_changed(uids) do
    ...
    if keys != [] do
      SREChat.PubSub.broadcast_system(keys, %{"type" => "membership_changed"})
    end

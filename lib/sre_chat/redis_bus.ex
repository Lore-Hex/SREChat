defmodule SREChat.RedisBus do
  ...
  alias SREChat.RedisBus.PublisherLane

  @publisher_names [
    SREChat.RedisBusPublisher,
    SREChat.RedisBusPublisher1,
    SREChat.RedisBusPublisher2,
    SREChat.RedisBusPublisher3,
    SREChat.RedisBusPublisher4,
    SREChat.RedisBusPublisher5,
    SREChat.RedisBusPublisher6,
    SREChat.RedisBusPublisher7,
    SREChat.RedisBusPublisher8,
    SREChat.RedisBusPublisher9,
    SREChat.RedisBusPublisher10,
    SREChat.RedisBusPublisher11,
    SREChat.RedisBusPublisher12,
    SREChat.RedisBusPublisher13,
    SREChat.RedisBusPublisher14,
    SREChat.RedisBusPublisher15
  ]
  ...
  def init(_opts) do
    ...
    case Config.redis_url() do
      url when is_binary(url) and url != "" ->
        ...
        case RedisClient.pubsub_start_link(url, name: SREChat.RedisPubSub) do
          {:ok, pubsub} ->
  ...
  def handle_info(
    ...
    with {:ok, %{"origin" => origin, "keys" => keys, "event" => event} = decoded} <-
      ...
      if decoded["system"] == true do
        SREChat.PubSub.local_system_broadcast(keys, event)
      else
        SREChat.PubSub.local_broadcast(keys, event)
      end

      keys
      |> SREChat.PubSub.local_subscribed_keys()
  ...
  defp refresh_store(keys, event) do
    SREChat.Store.refresh_from_pubsub(keys, event)
  catch

defmodule SREChat.Observability do
  @moduledoc false
  use GenServer
  ...
  defp snapshot(state) do
    %{
      "service" => %{
        "name" => "openchat",
        "version" => SREChat.Config.version(),
        "host" => SREChat.Config.host(),
        "startedAt" => DateTime.to_iso8601(state.started_at),
        "uptimeSeconds" => DateTime.diff(DateTime.utc_now(), state.started_at, :second),
        "redisConfigured" => SREChat.Store.RedisPersistence.configured?(),
        "redisEnabled" => SREChat.Store.RedisPersistence.enabled?(),
        "redisBootMode" => SREChat.Config.redis_boot_mode()
      },
  ...
  defp empty_snapshot do
    %{
      "service" => %{"name" => "openchat", "version" => SREChat.Config.version()},
      "counters" => %{},
      "gauges" => %{},
      "histograms" => %{}
    }

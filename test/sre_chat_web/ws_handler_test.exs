defmodule SREChatWeb.WSHandlerTest do
  use ExUnit.Case, async: false

  alias SREChat.Store
  alias SREChatWeb.WSHandler

  setup do
    Store.reset!()
    SREChat.Observability.reset!()
    :ok
  end
  ...
  test "auth event accepts cached SDK credentials past their advisory expiry" do
    ...
    cached_jwt =
      SREChat.Store.AuthTokens.local_jwt(
        "cached-mobile-user",
        payload["authToken"],
        SREChat.Time.now() - 90_000
      )
    ...
    assert SREChat.Observability.snapshot()["counters"][
  ...
  test "auth event rejects cached local credentials after signing-secret rotation" do
    previous_secret = Application.get_env(:sre_chat, :local_jwt_secret)
    ...
    Application.put_env(:sre_chat, :local_jwt_secret, "ws-secret-a")
    assert {:ok, payload} = Store.create_auth_token("mobile-alice")
    token = payload["authToken"]
    jwt = SREChat.Store.AuthTokens.local_jwt("mobile-alice", token, SREChat.Time.now() - 90_000)

    Application.put_env(:sre_chat, :local_jwt_secret, "ws-secret-b")
  ...
  test "authenticated sockets resync group subscriptions after join and leave" do
    ...
    assert_receive {:sre_chat_system_event, %{"type" => "membership_changed"} = event}
    assert {:ok, state} = WSHandler.websocket_info({:sre_chat_system_event, event}, state)
  ...
  test "sdk join for existing members resyncs stale websocket group subscriptions" do
    ...
    SREChat.PubSub.unsubscribe({:group, "ws-existing-member-room"})
  ...
  test "websocket heartbeat sends protocol pings and keeps the timer replaceable" do
    previous = Application.get_env(:sre_chat, :websocket_heartbeat_ms)

    on_exit(fn ->
      case previous do
        nil -> Application.delete_env(:sre_chat, :websocket_heartbeat_ms)
        value -> Application.put_env(:sre_chat, :websocket_heartbeat_ms, value)
      end
    end)

    Application.put_env(:sre_chat, :websocket_heartbeat_ms, 25_000)
  ...
  test "websocket heartbeat can be disabled for controlled deployments" do
    ...
    Application.put_env(:sre_chat, :websocket_heartbeat_ms, 0)
  ...
  test "websocket origin follows the configured origin allowlist" do
    previous = Application.get_env(:sre_chat, :cors_allowed_origins)

    on_exit(fn ->
      case previous do
        nil -> Application.delete_env(:sre_chat, :cors_allowed_origins)
        value -> Application.put_env(:sre_chat, :cors_allowed_origins, value)
      end
    end)
  ...
  test "authenticated read receipts update unread counts and broadcast a timestamped receipt" do
    ...
    SREChat.PubSub.subscribe({:user, "bob"})
  ...
  test "authenticated receipt store failures do not close the websocket" do
    previous = Application.get_env(:sre_chat, :redis_url)

    on_exit(fn ->
      case previous do
        nil -> Application.delete_env(:sre_chat, :redis_url)
        value -> Application.put_env(:sre_chat, :redis_url, value)
      end
    end)

    Application.put_env(:sre_chat, :redis_url, "redis://configured-but-unavailable")

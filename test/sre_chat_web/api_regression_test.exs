defmodule SREChatWeb.ApiRegressionTest do
  ...
  test "a doubled API-version prefix resolves to the same route" do
    # The CometChat iOS SDK prepends chatAPIVersion to message calls while the
    # host already carries /v3.0, so fetches arrive as /v3.0/v3.0/users/.../
    # messages. That 404'd and the SDK's callback hung. Both shapes must resolve.
    {:ok, _} = SREChat.Store.upsert_user(%{"uid" => "dbl", "name" => "Dbl"})

    single = auth_conn(:get, "/v3.0/users/dbl", %{}, "uid:alice")
    doubled = auth_conn(:get, "/v3.0/v3.0/users/dbl", %{}, "uid:alice")

    assert single.status == 200
    assert doubled.status == 200
    assert json(doubled)["data"]["uid"] == "dbl"

    # /v3 doubled too, and a single prefix must be untouched (idempotent).
    assert auth_conn(:get, "/v3/v3/users/dbl", %{}, "uid:alice").status == 200
    assert auth_conn(:get, "/v3.0/users/dbl", %{}, "uid:alice").status == 200
  end

  test "auth, admin, route fallback, and CORS failures are explicit" do
    conn = conn(:get, "/health") |> SREChatWeb.Endpoint.call([])
    assert conn.status == 200
    assert conn.resp_body == "ok"

    conn = conn(:get, "/v3.0/users") |> SREChatWeb.Endpoint.call([])
    ...
    conn = conn(:get, "/v3.0/does-not-exist") |> SREChatWeb.Endpoint.call([])
    assert conn.status == 404
    assert json(conn)["error"]["code"] == "ERR_ROUTE_NOT_FOUND"

    conn = conn(:delete, "/v3.0/me") |> SREChatWeb.Endpoint.call([])
    assert conn.status == 401
    assert json(conn)["error"]["code"] == "ERR_NO_AUTH"

    conn = conn(:options, "/v3.0/users") |> SREChatWeb.Endpoint.call([])
  ...
  test "blank configured admin api key does not open admin routes" do
    old_api_key = Application.get_env(:sre_chat, :api_key)
    Application.put_env(:sre_chat, :api_key, "")

    on_exit(fn ->
      Application.put_env(:sre_chat, :api_key, old_api_key)
    end)
  ...
  test "CORS origin allowlist can be restricted" do
    old_allowed_origins = Application.get_env(:sre_chat, :cors_allowed_origins)
    Application.put_env(:sre_chat, :cors_allowed_origins, "https://chat.example")

    on_exit(fn ->
      Application.put_env(:sre_chat, :cors_allowed_origins, old_allowed_origins)
    end)
  ...
  test "REST message mutations propagate SDK resource header to socket deviceId" do
    assert {:ok, _} = SREChat.PubSub.subscribe({:user, "alice"})
    assert {:ok, _} = SREChat.PubSub.subscribe({:user, "bob"})

    on_exit(fn ->
      SREChat.PubSub.unsubscribe({:user, "alice"})
      SREChat.PubSub.unsubscribe({:user, "bob"})
    end)
  ...
  test "media upload route stores files, serves them, and returns 404 for missing media" do
    old_upload_dir = Application.get_env(:sre_chat, :upload_dir)
    old_upload_max_bytes = Application.get_env(:sre_chat, :upload_max_bytes)
    ...
    Application.put_env(:sre_chat, :upload_dir, upload_dir)
    Application.put_env(:sre_chat, :upload_max_bytes, 1_000)

    on_exit(fn ->
      Application.put_env(:sre_chat, :upload_dir, old_upload_dir)
      Application.put_env(:sre_chat, :upload_max_bytes, old_upload_max_bytes)
    ...
    conn = conn(:get, "/v3.0#{media_path}") |> SREChatWeb.Endpoint.call([])
    assert conn.status == 200
    assert conn.resp_body == "uploaded text"

    conn = conn(:get, "/v3.0/media/missing.txt") |> SREChatWeb.Endpoint.call([])
    assert conn.status == 404
    ...
    conn = conn(:get, "/v3.0/media/#{unsafe_media}") |> SREChatWeb.Endpoint.call([])
    assert conn.status == 404
    assert json(conn)["error"]["code"] == "ERR_MEDIA_NOT_FOUND"
    ...
    Application.put_env(:sre_chat, :upload_max_bytes, 4)
  ...
  test "media upload route stores S3-backed files behind signed URLs only" do
    old_media_storage = Application.get_env(:sre_chat, :media_storage)
    old_s3_bucket = Application.get_env(:sre_chat, :s3_bucket)
    old_s3_client = Application.get_env(:sre_chat, :s3_client)
    old_s3_ttl = Application.get_env(:sre_chat, :s3_presigned_url_ttl_seconds)
    old_public_media_base_url = Application.get_env(:sre_chat, :public_media_base_url)
    ...
    SREChat.MockS3.reset()
    Application.put_env(:sre_chat, :media_storage, "s3")
    Application.put_env(:sre_chat, :s3_bucket, "openchat-api-test-uploads")
    Application.put_env(:sre_chat, :s3_client, SREChat.MockS3)
    Application.put_env(:sre_chat, :s3_presigned_url_ttl_seconds, 1200)
    Application.put_env(:sre_chat, :public_media_base_url, "https://openchat.example")

    on_exit(fn ->
      Application.put_env(:sre_chat, :media_storage, old_media_storage)
      Application.put_env(:sre_chat, :s3_bucket, old_s3_bucket)
      Application.put_env(:sre_chat, :s3_client, old_s3_client)
      Application.put_env(:sre_chat, :s3_presigned_url_ttl_seconds, old_s3_ttl)
      Application.put_env(:sre_chat, :public_media_base_url, old_public_media_base_url)
      SREChat.MockS3.reset()
      File.rm(source_path)
    end)
    ...
    conn = conn(:get, "/v3.0/media/#{Path.basename(uri.path)}") |> SREChatWeb.Endpoint.call([])
    assert conn.status == 404
    assert json(conn)["error"]["code"] == "ERR_MEDIA_NOT_FOUND"
  end

  test "settings, JWT, user sessions, and logout token revocation routes work together" do
    conn = conn(:get, "/v3.0/settings") |> SREChatWeb.Endpoint.call([])
    assert conn.status == 200
  ...
  test "TTFM socket admin-POST text message: data.metadata.chatMessage envelope and <@uid:>/<consumableId:> mentions round-trip" do
    ...
    # inside the visible text. SREChat must accept this shape and return it
  ...
  test "history and live payloads do not expose malformed legacy media messages to the SDK" do
    ...
    assert {:ok, _subscription} = SREChat.PubSub.subscribe({:user, "bob"})
  ...
  test "rooms-service deleteRoomConversation clears group history without deleting room or members" do
    ...
    # conversation id. SREChat must accept that alias and keep the room usable.

defmodule SREChat.ApplicationTest do
  use ExUnit.Case, async: false

  alias SREChat.Application, as: SREChatApplication
  ...
  test "local media storage creates the upload directory only when explicitly allowed" do
    ...
    assert :ok = SREChatApplication.ensure_media_storage!()
  ...
  test "local media storage is rejected when disabled for production-like environments" do
    ...
    assert_raise ArgumentError, ~r/MEDIA_STORAGE=local is not allowed/, fn ->
      SREChatApplication.ensure_media_storage!()
    end
  ...
  test "s3 media storage requires a bucket and skips durable local upload setup" do
    ...
    Application.put_env(:sre_chat, :s3_bucket, "openchat-test-media")
    Application.put_env(:sre_chat, :upload_dir, upload_dir)
  ...
  test "production-like security config rejects weak non-empty admin API keys" do
    Application.put_env(:sre_chat, :reject_weak_admin_api_key, true)

    Application.put_env(:sre_chat, :api_key, "")
    assert :ok = SREChatApplication.ensure_security_config!()

    for weak <- ["None", "null", "short"] do
      Application.put_env(:sre_chat, :api_key, weak)

      assert_raise ArgumentError, ~r/COMETCHAT_API_KEY must be blank/, fn ->
        SREChatApplication.ensure_security_config!()
      end
    end

    Application.put_env(:sre_chat, :api_key, String.duplicate("a", 32))
    assert :ok = SREChatApplication.ensure_security_config!()
  end
end

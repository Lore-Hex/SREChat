defmodule SREChat.Store.RetentionTest do
  use ExUnit.Case, async: false

  alias SREChat.Time
  alias SREChat.Store.{Conversations, Retention, State}
  ...
  defp with_env(values, fun) do
    previous =
      Map.new(values, fn {key, _value} -> {key, Application.get_env(:sre_chat, key)} end)

    Enum.each(values, fn {key, value} -> Application.put_env(:sre_chat, key, value) end)

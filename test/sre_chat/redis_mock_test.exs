defmodule SREChat.RedisMockTest do
  use ExUnit.Case, async: false

  alias SREChat.{MockRedis, Observability, RedisBus, Store}
  alias SREChat.Store.{RedisPersistence, RequestPlan, State}
  ...
  setup do
    previous =
      Map.new(
        ...
        &{&1, Application.get_env(:sre_chat, &1)}
      )

    stop_redis_clients()
    stop_name(SREChat.RedisPubSub)
    stop_redis_publishers()

    Application.put_env(:sre_chat, :redis_url, "mock://redis")
    Application.put_env(:sre_chat, :redis_key_prefix, "mock:test")
    Application.put_env(:sre_chat, :redis_snapshot_key, "mock:test:legacy_snapshot")
    Application.put_env(:sre_chat, :redis_boot_mode, "full")
    Application.put_env(:sre_chat, :redis_client, MockRedis)
    Application.put_env(:sre_chat, :redis_pubsub_client, MockRedis.PubSub)
  ...
  test "Redis fallback and error branches return safe values" do
    ...
    Application.put_env(:sre_chat, :redis_url, "")
    assert RedisPersistence.load_or_seed(default, seed) == seed.()

    Application.put_env(:sre_chat, :redis_url, "mock://redis")
    Application.put_env(:sre_chat, :redis_client, MockRedis.FailingClient)
    stop_redis_clients()
    ...
    Application.put_env(:sre_chat, :redis_client, MockRedis.AlreadyStartedClient)
    stop_redis_clients()
    assert RedisPersistence.load_or_seed(default, seed) == seed.()
  ...
  test "mock Redis covers bucket decode and refresh edge cases" do
    ...
    assert {:ok, _} =
             MockRedis.command(SREChat.Redis, [
               "SADD",
    ...
    assert {:ok, "OK"} =
             MockRedis.command(SREChat.Redis, ["SET", "mock:test:nx", "one", "NX", "PX", "10"])

    assert {:ok, nil} =
             MockRedis.command(SREChat.Redis, ["SET", "mock:test:nx", "two", "NX", "PX", "10"])

    assert {:ok, 1} = MockRedis.command(SREChat.Redis, ["SADD", "mock:test:set", "a"])
    assert {:ok, 1} = MockRedis.command(SREChat.Redis, ["SREM", "mock:test:set", "a"])
    assert {:ok, 1} = MockRedis.command(SREChat.Redis, ["DEL", "mock:test:nx"])

    MockRedis.put_string("mock:test:int", 4)
    assert {:ok, 5} = MockRedis.command(SREChat.Redis, ["INCR", "mock:test:int"])

    MockRedis.force_pipeline({:ok, [:forced]})
    assert MockRedis.pipeline(SREChat.Redis, [["GET", "mock:test:int"]]) == {:ok, [:forced]}

    assert MockRedis.command(SREChat.Redis, ["EVAL", "return 1", "1", "mock:test:int", "1"]) ==
             {:error, {:unsupported, ["EVAL", "return 1", "1", "mock:test:int", "1"]}}

    assert MockRedis.command(SREChat.Redis, ["NOOP"]) == {:error, {:unsupported, ["NOOP"]}}
  end
  ...
  test "lazy Redis boot skips full startup load and uses targeted refreshes" do
    ...
    Application.put_env(:sre_chat, :redis_boot_mode, "lazy")
    MockRedis.put_string("mock:test:meta:version", "7")
  ...
  test "RedisBus publishes to mock Redis and consumes remote pubsub events" do
    ...
    {:ok, _} = SREChat.PubSub.subscribe({:user, "alice"})
    {:ok, _} = SREChat.PubSub.subscribe({:group, "room"})
    ...
    assert_receive {:sre_chat_system_event, %{"type" => "system"}}
  ...
  test "RedisBus delivers remote events before slow store refresh completes" do
    ...
    {:ok, _} = SREChat.PubSub.subscribe({:user, "slow-refresh"})
  ...
  test "PubSub locally delivers even when Redis publish fails" do
    ...
    {:ok, _} = SREChat.PubSub.subscribe({:user, "ordered"})

    first_event = %{"text" => "first"}
    connection = publisher_connection([{:user, "ordered"}], first_event)

    assert :ok = SREChat.PubSub.broadcast({:user, "ordered"}, first_event)
    ...
    assert :ok = SREChat.PubSub.broadcast({:user, "ordered"}, %{"text" => "local-only"})
  ...
  test "publisher lanes prevent one slow room from delaying unrelated rooms" do
    Observability.reset!()
    Application.put_env(:sre_chat, :redis_publisher_lanes, 4)
    start_mock_redis()
    terminate_redis_bus()
    restart_redis_bus()
  ...
  test "RedisBus init handles already-started and failed pubsub clients" do
    start_mock_redis()
    {:ok, pubsub} = MockRedis.PubSub.start_link("mock://redis", name: SREChat.RedisPubSub)

    assert {:ok, %{pubsub: ^pubsub}} = RedisBus.init([])

    assert {:error, {:already_started, _pid}} =
             MockRedis.AlreadyStartedClient.start_link("mock://redis", name: SREChat.Redis)

    stop_name(SREChat.RedisPubSub)
    Application.put_env(:sre_chat, :redis_pubsub_client, MockRedis.FailingPubSub)
  ...
  defp start_mock_redis do
    case Process.whereis(SREChat.Redis) do
      nil -> {:ok, _pid} = MockRedis.start_link("mock://redis", name: SREChat.Redis)
      _pid -> :ok
    end
  end
  ...
  defp terminate_redis_bus do
    case Process.whereis(RedisBus) do
      nil -> :ok
      _pid -> Supervisor.terminate_child(SREChat.Supervisor, RedisBus)
    end
  end

  defp restart_redis_bus do
    case Supervisor.restart_child(SREChat.Supervisor, RedisBus) do
      {:ok, _pid} -> :ok
      {:ok, _pid, _info} -> :ok

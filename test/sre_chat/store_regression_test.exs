defmodule SREChat.StoreRegressionTest do
  use ExUnit.Case, async: false

  alias SREChat.{Store, Time}
  ...
  test "local JWTs reject malformed, tampered, and re-signed tokens" do
    old_secret = Application.get_env(:sre_chat, :local_jwt_secret)

    on_exit(fn ->
      Application.put_env(:sre_chat, :local_jwt_secret, old_secret)
    end)

    Application.put_env(:sre_chat, :local_jwt_secret, "jwt-secret-a")
  ...
  test "uid developer tokens are runtime gated and not embedded in default seed users" do
    old_accept_uid_tokens = Application.fetch_env!(:sre_chat, :accept_uid_tokens)

    on_exit(fn ->
      Application.put_env(:sre_chat, :accept_uid_tokens, old_accept_uid_tokens)
      Store.reset!()
    end)

    Application.put_env(:sre_chat, :accept_uid_tokens, false)
    Store.reset!()

    assert {:error, %{"code" => "ERR_NO_AUTH"}} = Store.me("uid:alice")
    assert {:error, %{"code" => "ERR_NO_AUTH"}} = Store.me("uid:not-seeded")

    Application.put_env(:sre_chat, :accept_uid_tokens, true)
    Store.reset!()
  ...
  test "banned users cannot read a public group even when public reads are enabled" do
    with_sre_chat_env(%{public_group_reads_enabled: true}, fn ->
  ...
  test "public_group_reads_enabled=false denies non-member message reads" do
    with_sre_chat_env(%{public_group_reads_enabled: false}, fn ->
  ...
  test "media uploads use configured upload dir and public media base URL" do
    ...
    old_media_base_url = Application.get_env(:sre_chat, :public_media_base_url)
    old_upload_max_bytes = Application.get_env(:sre_chat, :upload_max_bytes)
    old_allowed_mime_types = Application.get_env(:sre_chat, :upload_allowed_mime_types)
    ...
    Application.put_env(:sre_chat, :public_media_base_url, "https://media.example")
    ...
    on_exit(fn ->
      ...
      Application.put_env(:sre_chat, :public_media_base_url, old_media_base_url)
      Application.put_env(:sre_chat, :upload_max_bytes, old_upload_max_bytes)
      Application.put_env(:sre_chat, :upload_allowed_mime_types, old_allowed_mime_types)
  ...
  test "delivery receipts persist a delivered cursor and notify the sender" do
    ...
    SREChat.PubSub.subscribe({:user, "alice"})

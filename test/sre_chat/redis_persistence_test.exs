defmodule SREChat.RedisPersistenceTest do
  use ExUnit.Case, async: false

  alias SREChat.Store
  alias SREChat.Store.{Conversations, RedisPersistence}
  ...
  setup context do
    if reason = context[:redis_unavailable] do
      ...
      prefix = "sre_chat:test:#{System.unique_integer([:positive])}"
      ...
      Application.put_env(:sre_chat, :redis_url, context.redis_url)
      Application.put_env(:sre_chat, :redis_key_prefix, prefix)
      Application.put_env(:sre_chat, :redis_snapshot_key, snapshot_key)
      restart_store!()
  ...
  test "real Redis uses a separate writer connection for mutating commands", context do
    with_redis(context, fn ->
      assert reader = Process.whereis(SREChat.Redis)
      assert writer = Process.whereis(SREChat.RedisWriter)
      assert reader != writer
      assert Process.whereis(SREChat.RedisMutationReader)
      assert Process.whereis(SREChat.RedisCounter)
  ...
  test "mutation refreshes do not queue behind the normal read connection", context do
    with_redis(context, fn ->
      assert Process.whereis(SREChat.Redis)
      assert Process.whereis(SREChat.RedisWriter)
      assert Process.whereis(SREChat.RedisMutationReader)
      ...
      blocked_reader =
        Task.async(fn ->
          SREChat.RedisClient.command(SREChat.Redis, ["BLPOP", blocking_key, "1"])
        end)
  ...
  test "message history reads load a bounded Redis page without warming the Store mailbox",
       context do
    with_redis(context, fn ->
      with_sre_chat_env(%{redis_conversation_refresh_limit: 20}, fn ->
        conv_id = Conversations.user_conversation_id("bounded-a", "bounded-b")
        now = SREChat.Time.now()
  ...
  test "conversation cleanup refreshes full Redis history before deleting message records",
       context do
    with_redis(context, fn ->
      with_sre_chat_env(%{redis_boot_mode: "lazy"}, fn ->
  ...
  test "Redis write-through trims retained group histories and evicted message keys",
       context do
    with_redis(context, fn ->
      with_sre_chat_env(%{group_max_messages: 3, group_message_retention_days: 365}, fn ->
  ...
  test "transient public-room presence fans out across peer Store processes", context do
    with_redis(context, fn ->
      ...
      try do
        ...
        {:ok, _subscription} = SREChat.PubSub.subscribe({:user, "presence-viewer"})
        ...
        assert get_in(:sys.get_state(peer), ["presence", guid, "presence-viewer", "uid"]) ==
                 "presence-viewer"
      after
        SREChat.PubSub.unsubscribe({:user, "presence-viewer"})
        stop_peer_store(peer)
      end
    end)
  end
  ...
  test "group membership action messages use Redis-backed IDs without reserved-offset collisions",
    ...
    with_redis(context, fn ->
      ...
      assert {:ok, _subscription} = SREChat.PubSub.subscribe({:user, "redis-observer"})
  ...
  test "region allocation over shared Redis: two stores in one region never collide", context do
    with_redis(context, fn ->
      with_sre_chat_env(%{id_allocator: "region", region_index: 5}, fn ->
        peer = start_peer_store!()

        try do
          ids =
            for i <- 1..40 do
              server = if rem(i, 2) == 0, do: SREChat.Store, else: peer
          ...
          for id <- ids do
            {_ms, region, _seq} = SREChat.RegionId.decompose(id)
            assert region == 5
  ...
  test "region sequence slots are 0-based, increasing, and expire", context do
    with_redis(context, fn ->
      ...
      prefix = Application.fetch_env!(:sre_chat, :redis_key_prefix)
  ...
  describe "oplog emission" do
      ...
      with_redis(context, fn ->
        with_sre_chat_env(
          ...
          fn ->
            ...
            SREChat.Replication.while_applying(fn ->
              :ok =
                RedisPersistence.write([
    ...
    test "replication off emits nothing", context do
      with_redis(context, fn ->
        with_sre_chat_env(%{id_allocator: "region", region_index: 1}, fn ->
          {:ok, _} =
    ...
    test "multi_master without region ids refuses to boot" do
      previous_mode = Application.get_env(:sre_chat, :replication_mode)
      previous_allocator = Application.get_env(:sre_chat, :id_allocator)

      on_exit(fn ->
        Application.put_env(:sre_chat, :replication_mode, previous_mode || "off")
        Application.put_env(:sre_chat, :id_allocator, previous_allocator || "global")
      end)

      Application.put_env(:sre_chat, :replication_mode, "multi_master")
      ...
      assert SREChat.Replication.ensure_valid_config!() == :ok
    end
  end
  ...
  defp oplog_entries(context) do
    ...
    Enum.map(raw, fn [_id, ["entry", json]] ->
      {:ok, entry} = SREChat.Replication.decode_entry(json)
      entry
    end)
  end
  ...
  defp redis_connection_names do
    [SREChat.Redis, SREChat.RedisWriter, SREChat.RedisMutationReader, SREChat.RedisCounter] ++
      redis_lock_names()
  end

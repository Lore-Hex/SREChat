defmodule SREChat.StoreGroupStateTest do
  use ExUnit.Case, async: false

  alias SREChat.Time
  alias SREChat.Store.{Conversations, Entities, GroupState, State}
  ...
  test "transient joins require public non-member visitor semantics" do
    with_sre_chat_env(%{public_group_joins_as_visits: false}, fn ->
  ...
  test "member limits reject only new members over the configured cap" do
    with_sre_chat_env(%{group_max_members: 2}, fn ->
      state =
        State.default()
        |> put_group("room")
        |> GroupState.add_member("room", "alice", "participant")
  ...
  test "presence updates purge expired rows and cap large public rooms" do
    ...
    with_sre_chat_env(%{group_presence_ttl_seconds: 10, group_max_presence: 2}, fn ->
      state =
        State.default()
        |> put_group("room")
  ...
  defp with_sre_chat_env(overrides, fun) do
    old = Map.new(overrides, fn {key, _value} -> {key, Application.get_env(:sre_chat, key)} end)
    ...
    after
      Enum.each(old, fn {key, value} ->
        if is_nil(value),
          do: Application.delete_env(:sre_chat, key),
          else: Application.put_env(:sre_chat, key, value)
      end)
    end
  end
end

defmodule SREChat.LoadPerfTest do
  ...
  setup do
    old_redis_url = Application.get_env(:sre_chat, :redis_url)
    Application.put_env(:sre_chat, :redis_url, nil)
    restart_store!()

    on_exit(fn ->
      Application.put_env(:sre_chat, :redis_url, old_redis_url)
      restart_store!()
    end)
  ...
  test "store sustains high-volume direct message writes and paginated reads" do
    users = env_int("SRECHAT_LOAD_USERS", 100)
    messages = env_int("SRECHAT_LOAD_MESSAGES", 2_000)
    minimum = env_int("SRECHAT_MIN_STORE_MSG_PER_SEC", 300)
  ...
  test "concurrent Store writers preserve monotonic IDs under same-conversation contention" do
    concurrency = env_int("SRECHAT_LOAD_CONCURRENCY", 16)
    per_worker = env_int("SRECHAT_LOAD_WORKER_MESSAGES", 150)
    total = concurrency * per_worker
    minimum = env_int("SRECHAT_MIN_CONCURRENT_STORE_MSG_PER_SEC", 200)
  ...
  test "deep conversations keep unread and conversation reads on cursor indexes" do
    messages = env_int("SRECHAT_LOAD_DEEP_CONVERSATION_MESSAGES", 2_000)
    reads = env_int("SRECHAT_LOAD_DEEP_CONVERSATION_READS", 600)
    minimum = env_int("SRECHAT_MIN_DEEP_CURSOR_READ_PER_SEC", 300)
    ...
    {results, seconds} =
      timed(fn ->
        concurrent_map(1..reads, env_int("SRECHAT_LOAD_DEEP_READ_CONCURRENCY", 16), fn _i ->
  ...
  test "group fanout plus read and delivered receipts keep conversation cursors stable" do
    members = env_int("SRECHAT_LOAD_GROUP_MEMBERS", 150)
    messages = env_int("SRECHAT_LOAD_GROUP_MESSAGES", 600)
    minimum = env_int("SRECHAT_MIN_GROUP_MSG_PER_SEC", 100)
    guid = "load-room"
    ...
    {_receipts, receipt_seconds} =
      timed(fn ->
        concurrent_map(receipt_users, env_int("SRECHAT_LOAD_RECEIPT_CONCURRENCY", 16), fn uid ->
    ...
    assert_rate(
      ...
      receipt_seconds,
      env_int("SRECHAT_MIN_RECEIPT_PER_SEC", 100)
    )
  ...
  test "top public room load keeps transient visits, fanout, and history bounded" do
    durable_members = env_int("SRECHAT_LOAD_TOP_ROOM_DURABLE_MEMBERS", 80)
    visitors = env_int("SRECHAT_LOAD_TOP_ROOM_VISITORS", 1_000)
    messages = env_int("SRECHAT_LOAD_TOP_ROOM_MESSAGES", 800)
    retained = env_int("SRECHAT_LOAD_TOP_ROOM_RETAINED_MESSAGES", 120)
    fanout_limit = env_int("SRECHAT_LOAD_TOP_ROOM_FANOUT_LIMIT", 20)
    concurrency = env_int("SRECHAT_LOAD_TOP_ROOM_CONCURRENCY", 32)
    minimum = env_int("SRECHAT_MIN_TOP_ROOM_OP_PER_SEC", 150)
  ...
  test "HTTP endpoint sustains message send and fetch load through Plug" do
    messages = env_int("SRECHAT_LOAD_HTTP_MESSAGES", 500)
    minimum = env_int("SRECHAT_MIN_HTTP_MSG_PER_SEC", 100)
  ...
  test "concurrent HTTP endpoint load preserves unique message IDs" do
    concurrency = env_int("SRECHAT_LOAD_HTTP_CONCURRENCY", 12)
    per_worker = env_int("SRECHAT_LOAD_HTTP_WORKER_MESSAGES", 50)
    total = concurrency * per_worker
    minimum = env_int("SRECHAT_MIN_CONCURRENT_HTTP_MSG_PER_SEC", 100)
  ...
  test "HTTP auth-token history load preserves visibility under concurrent reads" do
    with_redis(fn _context ->
      messages = env_int("SRECHAT_LOAD_HTTP_AUTH_MESSAGES", 300)
      reads = env_int("SRECHAT_LOAD_HTTP_AUTH_READS", 120)
      concurrency = env_int("SRECHAT_LOAD_HTTP_AUTH_READ_CONCURRENCY", 24)
      min_send_rate = env_int("SRECHAT_MIN_HTTP_AUTH_MSG_PER_SEC", 20)
      min_read_rate = env_int("SRECHAT_MIN_HTTP_AUTH_READ_PER_SEC", 20)
      max_read_p95_ms = env_int("SRECHAT_MAX_HTTP_AUTH_HISTORY_P95_MS", 750)
  ...
  test "concurrent reaction load preserves rows, summaries, extension metadata, and history" do
    users = env_int("SRECHAT_LOAD_REACTION_USERS", 36)
    messages = env_int("SRECHAT_LOAD_REACTION_MESSAGES", 30)
    concurrency = env_int("SRECHAT_LOAD_REACTION_CONCURRENCY", 24)
    minimum = env_int("SRECHAT_MIN_REACTION_OP_PER_SEC", 100)
    ...
    assert_group_reaction_state(SREChat.Store, viewer, guid, sent, ops)
    ...
    assert_group_reaction_state(SREChat.Store, viewer, guid, sent, after_remove)
    ...
    assert_group_reaction_state(SREChat.Store, viewer, guid, sent, final_ops)
  end

  test "Redis write-through load uses scoped refresh and monotonic counters" do
    with_redis(fn context ->
      messages = env_int("SRECHAT_LOAD_REDIS_MESSAGES", 300)
      minimum = env_int("SRECHAT_MIN_REDIS_MSG_PER_SEC", 20)
  ...
  test "concurrent Redis write-through load preserves counters and per-key indexes" do
    with_redis(fn context ->
      concurrency = env_int("SRECHAT_LOAD_REDIS_CONCURRENCY", 8)
      per_worker = env_int("SRECHAT_LOAD_REDIS_WORKER_MESSAGES", 60)
      total = concurrency * per_worker
      minimum = env_int("SRECHAT_MIN_CONCURRENT_REDIS_MSG_PER_SEC", 20)
  ...
  test "concurrent Redis write-through across peer stores scales on independent conversations" do
    with_redis(fn context ->
      ...
      try do
        lanes = env_int("SRECHAT_LOAD_REDIS_PEER_LANES", 16)
        concurrency = env_int("SRECHAT_LOAD_REDIS_PEER_CONCURRENCY", 10)
        per_lane = env_int("SRECHAT_LOAD_REDIS_PEER_MESSAGES_PER_LANE", 20)
        total = lanes * per_lane
        minimum = env_int("SRECHAT_MIN_REDIS_PEER_MSG_PER_SEC", 20)
  ...
  test "mixed Redis peer writes and cursor reads keep unread indexes hot" do
    with_redis(fn _context ->
      ...
      try do
        lanes = env_int("SRECHAT_LOAD_REDIS_MIXED_LANES", 8)
        per_lane = env_int("SRECHAT_LOAD_REDIS_MIXED_MESSAGES_PER_LANE", 20)
        concurrency = env_int("SRECHAT_LOAD_REDIS_MIXED_CONCURRENCY", 10)
        total = lanes * per_lane
        minimum = env_int("SRECHAT_MIN_REDIS_MIXED_OP_PER_SEC", 20)
  ...
  test "concurrent Redis secondary-index reads stay consistent under write-through load" do
    with_redis(fn context ->
      concurrency = env_int("SRECHAT_LOAD_REDIS_INDEX_CONCURRENCY", 8)
      messages = env_int("SRECHAT_LOAD_REDIS_INDEX_MESSAGES", 240)
      minimum = env_int("SRECHAT_MIN_REDIS_INDEX_OP_PER_SEC", 20)
  ...
  test "Redis peer reaction load preserves final state across stores and Redis" do
    with_redis(fn context ->
      users = env_int("SRECHAT_LOAD_REDIS_REACTION_USERS", 24)
      messages = env_int("SRECHAT_LOAD_REDIS_REACTION_MESSAGES", 16)
      concurrency = env_int("SRECHAT_LOAD_REDIS_REACTION_CONCURRENCY", 16)
      minimum = env_int("SRECHAT_MIN_REDIS_REACTION_OP_PER_SEC", 20)
  ...
  defp assert_direct_histories_recorded(messages),
    do: assert_direct_histories_recorded(SREChat.Store, messages)
  ...
  defp assert_group_history_recorded(viewer_uid, guid, expected),
    do: assert_group_history_recorded(SREChat.Store, viewer_uid, guid, expected)
  ...
  defp with_redis(fun) do
    ...
    case Redix.start_link(redis_url) do
      {:ok, redis} ->
        prefix = "sre_chat:load:#{System.unique_integer([:positive])}"
        old_redis_url = Application.get_env(:sre_chat, :redis_url)
        old_key_prefix = Application.get_env(:sre_chat, :redis_key_prefix)
        old_snapshot_key = Application.get_env(:sre_chat, :redis_snapshot_key)

        Application.put_env(:sre_chat, :redis_url, redis_url)
        Application.put_env(:sre_chat, :redis_key_prefix, prefix)
        Application.put_env(:sre_chat, :redis_snapshot_key, "#{prefix}:legacy_snapshot")
        ...
        after
          Application.put_env(:sre_chat, :redis_url, old_redis_url)
          Application.put_env(:sre_chat, :redis_key_prefix, old_key_prefix)
          Application.put_env(:sre_chat, :redis_snapshot_key, old_snapshot_key)
          restart_store!()
          delete_prefix(redis, prefix)
        end
  ...
  defp restart_store! do
    if Process.whereis(SREChat.Store) do
      :ok = Supervisor.terminate_child(SREChat.Supervisor, SREChat.Store)
    end

    stop_redis_clients()

    case Supervisor.restart_child(SREChat.Supervisor, SREChat.Store) do
      {:ok, _pid} -> :ok
      {:ok, _pid, _info} -> :ok
      {:error, {:already_started, _pid}} -> :ok
      other -> flunk("failed to restart SREChat.Store: #{inspect(other)}")
    end
  end
  ...
  defp stop_redis_clients do
    [
      SREChat.Redis,
      SREChat.RedisWriter,
      SREChat.RedisMutationReader,
      SREChat.RedisCounter,
      SREChat.RedisLock0,
      SREChat.RedisLock1,
      SREChat.RedisLock2,
      SREChat.RedisLock3

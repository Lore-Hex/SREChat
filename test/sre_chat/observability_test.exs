defmodule SREChat.ObservabilityTest do
  ...
  test "records counters, gauges, histograms, and sanitised HTTP paths" do
    SREChat.Observability.record_http("GET", "/v3.0/users/alice/messages?limit=5", 200, 42)
    SREChat.Observability.record_store_call("send_message", true, 17, "ok")

    SREChat.Observability.record_store_call("authenticate", false, 5, "error", %{
      "code" => "ERR_NO_AUTH"
    })

    SREChat.Observability.record_api_error(
      "GET",
    ...
    SREChat.Observability.record_auth_attempt("rest", "ERR_NO_AUTH", true)
    SREChat.Observability.record_redis_lock([{:conversation, "room"}, {:user, "alice"}], "ok", 3)
    SREChat.Observability.add_gauge("ws.active", 2)
    SREChat.Observability.record_ws("auth_success")

    Process.sleep(20)
    snapshot = SREChat.Observability.snapshot()

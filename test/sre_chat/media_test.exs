defmodule SREChat.MediaTest do
  ...
  setup do
    ...
    old_allow_local = Application.get_env(:sre_chat, :allow_local_media_storage)
    ...
    Application.put_env(:sre_chat, :media_storage, "local")
    Application.put_env(:sre_chat, :allow_local_media_storage, true)
  ...
  test "persist_upload with valid file" do
    ...
    old_max = Application.get_env(:sre_chat, :upload_max_bytes)
    Application.put_env(:sre_chat, :upload_max_bytes, 10_000_000)
    ...
    File.rm!(path)
    Application.put_env(:sre_chat, :upload_max_bytes, old_max)
  end

  test "persist_upload stores media in S3 when configured" do
    Application.put_env(:sre_chat, :media_storage, "s3")
    Application.put_env(:sre_chat, :s3_bucket, "openchat-test-uploads")
  ...
  test "sign_urls returns fresh signed S3 URLs for stored media references" do
    ...
    Application.put_env(:sre_chat, :s3_bucket, "openchat-test-uploads")
    Application.put_env(:sre_chat, :s3_client, SREChat.MockS3)
    Application.put_env(:sre_chat, :s3_presigned_url_ttl_seconds, 900)
    Application.put_env(:sre_chat, :public_media_base_url, "https://openchat.example")
  ...
  test "persist_upload rejects large files" do
    old_max = Application.get_env(:sre_chat, :upload_max_bytes)
    Application.put_env(:sre_chat, :upload_max_bytes, 5)
  ...
  test "persist_upload rejects invalid mime types" do
    old_allowed = Application.get_env(:sre_chat, :upload_allowed_mime_types)
    Application.put_env(:sre_chat, :upload_allowed_mime_types, ["image/png"])
    ...
    File.rm!(path)
    Application.put_env(:sre_chat, :upload_allowed_mime_types, old_allowed)
  end

  test "local media storage is disabled when the environment forbids it" do
    Application.put_env(:sre_chat, :allow_local_media_storage, false)
  ...
  test "unknown media storage modes do not fall back to local disk" do
    Application.put_env(:sre_chat, :media_storage, "shared-nfs")
  ...
  test "s3 media storage requires a bucket before reading upload temp files" do
    Application.put_env(:sre_chat, :media_storage, "s3")
    Application.delete_env(:sre_chat, :s3_bucket)
  ...
  test "media_url formats correctly" do
    ...
    old_base = Application.get_env(:sre_chat, :public_media_base_url)
    Application.put_env(:sre_chat, :public_media_base_url, "https://cdn.example.com")
    assert Media.media_url("test.jpg") == "https://cdn.example.com/media/test.jpg"

    Application.put_env(:sre_chat, :public_media_base_url, "https://cdn.example.com/")
    assert Media.media_url("test.jpg") == "https://cdn.example.com/media/test.jpg"

    Application.put_env(:sre_chat, :public_media_base_url, old_base)
  end
  ...
  defp restore_app_env(key, nil), do: Application.delete_env(:sre_chat, key)
  defp restore_app_env(key, value), do: Application.put_env(:sre_chat, key, value)
end

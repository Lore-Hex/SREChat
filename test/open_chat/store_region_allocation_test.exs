defmodule OpenChat.StoreRegionAllocationTest do
  use ExUnit.Case, async: false

  alias OpenChat.{RegionId, Store}

  setup do
    previous_allocator = Application.get_env(:open_chat, :id_allocator, "global")
    previous_region = Application.get_env(:open_chat, :region_index, 0)

    on_exit(fn ->
      Application.put_env(:open_chat, :id_allocator, previous_allocator)
      Application.put_env(:open_chat, :region_index, previous_region)
      Store.reset!()
    end)

    Application.put_env(:open_chat, :id_allocator, "region")
    Application.put_env(:open_chat, :region_index, 3)
    Store.reset!()
    :ok
  end
  ...
  test "the default allocator remains the legacy global counter" do
    ...
    # Small sequential ids, exactly as OpenChat allocates them.
